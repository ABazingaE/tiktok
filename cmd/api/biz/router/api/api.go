	...
	root := r.Group("/", rootMw()...)
	{
		...
		}
		{
			...
			}
			{
				...
				_list0.GET("/", append(_likelistMw(), api.LikeList)...)
			}
		}
		{
			_feed := _douyin.Group("/feed", _feedMw()...)
			_feed.GET("/", append(_videostreamMw(), api.VideoStream)...)
		}
		{
		...
		}
		{
			_publish := _douyin.Group("/publish", _publishMw()...)
			{
				...
				_action2.POST("/", append(_videouploadMw(), api.VideoUpload)...)
			}
			{
		...
		}
		{
			_relation := _douyin.Group("/relation", _relationMw()...)
			{
				...
				_action3.POST("/", append(_follow_ctionMw(), api.FollowAction)...)
			}
			{
